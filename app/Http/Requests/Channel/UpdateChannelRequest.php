...
{
    /**
     * Il faut être membre de la chaîne avec au moins une des permissions
     * d'édition (profil, apparence ou confidentialité) — owner/admin les ont
     * toujours toutes. Avant cette vérification, n'importe quel utilisateur
     * connecté pouvait modifier n'importe quelle chaîne.
     */
    public function authorize(): bool
    {
        ...
        return ChannelUser::userIsOwner($channelId, $user->id)
            || ChannelUser::userHasPermission($channelId, $user->id, 'channel.edit_profile')
            || ChannelUser::userHasPermission($channelId, $user->id, 'channel.edit_appearance')
            || ChannelUser::userHasPermission($channelId, $user->id, 'channel.edit_privacy');
    }
