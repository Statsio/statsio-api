...
class ProcessApiDataSourceEndToEndTest extends TestCase
{
    ...
    {
        // Ce test exerce la vraie conversion Parquet (DuckDbParquetWriter + assertions
        // `duckdb read_parquet`) : sans la CLI DuckDB il n'a rien à vérifier. Elle est
        // présente dans l'image CI mais pas sur les postes de dev (cf. DatasetControllerTest).
        if (! trim((string) shell_exec('command -v duckdb'))) {
            $this->markTestSkipped('DuckDB CLI absent — pipeline Parquet réel non testable ici.');
        }
